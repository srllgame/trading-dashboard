...
if uploaded_file:
    ...
    fig_symbol = px.bar(symbol_group, x="Symbol", y="Profit", title="Profit by Symbol", color="Profit", color_continuous_scale="Bluered")
    st.plotly_chart(fig_symbol, use_container_width=True)

    win_counts = df['Result'].value_counts()
    fig_donut = go.Figure(data=[go.Pie(labels=win_counts.index, values=win_counts.values, hole=.6)])
    ...
    heatmap_pivot = heatmap_pivot.loc[["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"]]
    fig_heatmap = px.imshow(heatmap_pivot, color_continuous_scale='RdYlGn', labels=dict(x="Hour", y="Weekday", color="Profit ($)"), title="Profit Heatmap by Weekday and Hour")
    ...
    fig_setup = px.bar(setup_stats, x="TotalProfit", y="Setup", orientation='h', title="Total Profit by Setup", color="WinRate", color_continuous_scale="greens")
    st.plotly_chart(fig_setup, use_container_width=True)

    st.subheader("📋 Trade Log")
    display_df = df[["Date", "Symbol", "Open price", "Close Price", "Side", "Profit", "Result", "Setup"]]
    st.dataframe(display_df.style.applymap(lambda v: 'color: lime;' if v == 'WIN' else ('color: red;' if v == 'LOSS' else None), subset=['Result']))
